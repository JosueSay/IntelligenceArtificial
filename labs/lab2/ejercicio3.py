...
def analizar_ley_benford(ruta_archivo):
    ...
    plt.figure(figsize=(10, 6))
    sns.ecdfplot(observadas, label='Observado', linestyle='--')
    sns.ecdfplot(teoricas, label='Benford', linestyle='-')
    plt.xlabel('Primer Dígito')
    plt.ylabel('Distribución Acumulativa')
    plt.title('Función de Distribución Acumulativa (CDF)')
    plt.legend()
    plt.grid()
    plt.savefig("./images/cdf_plot.png")
    print("Gráfico CDF guardado como './images/cdf_plot.png'")
    
    plt.figure(figsize=(10, 6))
    plt.scatter(np.sort(observadas), np.sort(teoricas))
    plt.plot([0, 1], [0, 1], 'r--')
    ...
    print("Gráfico QQ guardado como './images/qq_plot.png'")
